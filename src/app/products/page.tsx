...
export default function ProductsPage() {
  return (
    <>
      ...
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8"><AffiliateDisclosure /><div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">{database.products.map((product) => <DatabaseProductCard key={product.slug} product={product} affiliatePlacement="products-grid" />)}</div></section>
    </>
  );
}
